...
const RoleBadge = ({ role, isMySupervisor }) => {
    if (isMySupervisor)     return <span className="px-2 py-0.5 bg-zinc-900 text-white text-[10px] font-semibold rounded uppercase tracking-wide whitespace-nowrap">Your Supervisor</span>;
    if (role === "supervisor") return <span className="px-2 py-0.5 bg-zinc-100 text-zinc-700 text-[10px] font-semibold rounded uppercase tracking-wide whitespace-nowrap">Supervisor</span>;
    if (role === "hr")         return <span className="px-2 py-0.5 bg-amber-50 text-amber-700 text-[10px] font-semibold rounded uppercase tracking-wide whitespace-nowrap">HR</span>;
    return <span className="px-2 py-0.5 bg-zinc-50 text-zinc-500 text-[10px] font-semibold rounded uppercase tracking-wide whitespace-nowrap">Employee</span>;
};
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="md:hidden space-y-2">
                            {filtered.map(m => (
                                <div key={m.id} className="bg-white rounded-xl border border-zinc-200 p-4">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 ${m.is_my_supervisor ? "bg-zinc-900" : "bg-zinc-400"}`}>
                                            {initials(m.name)}
                                        </div>
                                        <div className="min-w-0 flex-1">
                                            <p className="font-medium text-zinc-800 text-sm truncate">{m.name}</p>
                                            <p className="text-xs text-zinc-400 truncate">{m.email}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-zinc-100">
                                        <span className="text-xs text-zinc-500">{m.service ?? "—"}</span>
                                        <RoleBadge role={m.role} isMySupervisor={m.is_my_supervisor} />
                                    </div>
                                </div>
                            ))}
                        </div>
