...
import { hrDocumentPending, hrDocumentApprove, hrDocumentReject } from "../services/hrService";
...
export default function HrDocumentsPage() {
    ...
    const load = () => {
        setLoading(true);
        hrDocumentPending()
            .then(r => setRequests(r.data.pending ?? []))
            .catch(() => {})
    ...
    const approve = async (id) => {
        setBusy(true);
        try { await hrDocumentApprove(id); load(); }
    ...
    const submitReject = async () => {
        ...
        setBusy(true);
        try { await hrDocumentReject(rejectId, reason); setRejectId(null); setReason(""); load(); }
        catch (err) { setError(err.response?.data?.error ?? "Failed."); }
        setBusy(false);
    };
        ...
        <DashboardLayout notifications={notifications} unreadCount={unreadCount} fetchNotifications={fetchNotifications}>
            <div className="space-y-4">
                ...
                {loading ? (
                    ...
                    <div className="bg-white rounded-xl border border-zinc-200 p-10 text-center text-sm text-zinc-400">No pending document requests.</div>
                ) : (
                    <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
                        <div className="px-6 py-4 border-b border-zinc-100">
                            <p className="text-sm font-semibold text-zinc-900">Pending Document Requests</p>
                            <p className="text-xs text-zinc-400 mt-0.5">{requests.length} awaiting approval</p>
                        </div>
                        <table className="w-full text-left text-sm">
                            <thead className="bg-zinc-50 border-b border-zinc-100">
                                <tr>{["Employee", "Document", "Reason", "Date", "Actions"].map(h => (
                            ...
                            <tbody className="divide-y divide-zinc-50">
                                {requests.map(r => (
                                    <tr key={r.id} className="hover:bg-zinc-50 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-zinc-500">{r.document}</td>
                                        <td className="px-4 py-3 text-zinc-400 max-w-xs truncate">{r.reason || "—"}</td>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex gap-2">
                                                <button onClick={() => approve(r.id)} disabled={busy}
                                                    className="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-medium rounded transition disabled:opacity-60">Approve</button>
                                                <button onClick={() => setRejectId(r.id)} disabled={busy}
                                                    className="px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-medium rounded transition disabled:opacity-60">Reject</button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
