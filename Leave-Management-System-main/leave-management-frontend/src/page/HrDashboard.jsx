...
import { hrPendingRequests, hrApprove, hrReject } from "../services/hrService";
...
export default function HrLeavePage() {
    ...
    const approve = async (id) => {
        setBusy(true);
        try { await hrApprove(id); load(); fetchNotifications(); }
    ...
    const submitReject = async () => {
        ...
        setBusy(true);
        try { await hrReject(rejectId, reason); setRejectId(null); setReason(""); load(); fetchNotifications(); }
        catch (err) { setError(err.response?.data?.error ?? "Failed."); }
        setBusy(false);
    };
        ...
        <DashboardLayout notifications={notifications} unreadCount={unreadCount} fetchNotifications={fetchNotifications}>
            <div className="space-y-4">
                ...
                {loading ? (
                    ...
                    <div className="bg-white rounded-xl border border-zinc-200 p-10 text-center text-sm text-zinc-400">No leave requests.</div>
                ) : (
                    <div className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
                        <div className="px-6 py-4 border-b border-zinc-100">
                            <p className="text-sm font-semibold text-zinc-900">Leave Approvals</p>
                            <p className="text-xs text-zinc-400 mt-0.5">{requests.filter(r => r.status === 'pending_hr').length} pending · {requests.filter(r => r.status === 'rejected').length} rejected</p>
                        </div>
                            ...
                            <tbody className="divide-y divide-zinc-50">
                                {requests.map(r => (
                                    <tr key={r.id} className={`transition-colors ${r.status === 'rejected' ? 'opacity-60' : 'hover:bg-zinc-50'}`}>
                                        ...
                                        <td className="px-4 py-3 text-zinc-500">{r.leave_type}</td>
